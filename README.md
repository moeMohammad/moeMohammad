<!-- Intro -->
<div align="center">
  <h3>Hi, I’m Mohammad 🦆</h3>
  <p>CS graduate who enjoys full-stack development, ML, and building whatever comes to mind.</p>

  <!-- Social / Links -->
  <a href="https://www.linkedin.com/in/mohammad-m-mohammad">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-Mohammad%20Mohammad-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
  </a>
</div>

---

<!-- Featured Projects -->
## 🌟 Featured Projects

### QuackTrack
A movie and TV tracking platform.

- Live: [quacktrack.tv](https://quacktrack.tv/)

<div>
  <a href="https://quacktrack.tv/">
    <img alt="Live Site" src="https://img.shields.io/badge/Live-quacktrack.tv-22c55e?style=for-the-badge&logo=google-chrome&logoColor=white">
  </a>
</div>

### UAEU Chat
A platform for students at UAEU to help each other and share ideas.

- Live: [uaeu.chat](https://uaeu.chat/)
- Repository: [AzizKri/uaeu-hub](https://github.com/AzizKri/uaeu-hub)

<div>
  <a href="https://uaeu.chat/">
    <img alt="Live Site" src="https://img.shields.io/badge/Live-uaeu.chat-22c55e?style=for-the-badge&logo=google-chrome&logoColor=white">
  </a>
  <a href="https://github.com/AzizKri/uaeu-hub">
    <img alt="Repo" src="https://img.shields.io/badge/GitHub-Repository-111827?style=for-the-badge&logo=github&logoColor=white">
  </a>
</div>

---

<!-- Skills -->
## 🧰 Skills
<div align="center">
  <img src="https://skillicons.dev/icons?i=py,ts,js,java,dart,html,css,react,flutter,spring,flask,sklearn,tensorflow,postgres,cloudflare&perline=8" alt="skills" />
</div>

- Languages: Python, TypeScript, JavaScript, Java, Dart, HTML, CSS  
- Frameworks/Libraries: React, Flutter, Spring Boot, Flask, scikit-learn, TensorFlow  
- Tools/Platforms: PostgreSQL, Cloudflare

---

<!-- Footer line -->
<div align="center">
  <sub>Thanks for visiting — always happy to connect!</sub>
</div>
